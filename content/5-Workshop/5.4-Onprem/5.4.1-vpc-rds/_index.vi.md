...
![Picture VPC Create 1](images/picCreateVPC1.jpg)
...
![Picture VPC Create 2](images/picCreateVPC2.jpg)
...
![Create NAT Gateway](images/picCreateNAT.jpg)
...
![Succes Create VPC and NAT Gateway](images/picVPC.jpg)

## Tạo RDS MySQL

### Chuẩn bị subnet group và parameter group

1. Trong **Amazon RDS → Subnet groups**, chọn **Create DB Subnet Group**, đặt tên `neonfoodmap-rds-subnet-group`, chọn VPC NeonFoodMap và private subnet của ít nhất hai AZ. ![picDbSubnetGroup](images/picDbSubnetGroup.jpg)
2. Trong **Parameter groups**, tạo `neonfoodmap-mysql80-params` cho MySQL 8.0/family tương ứng. Sửa `character_set_server=utf8mb4` và `collation_server=utf8mb4_unicode_ci` để xử lý dữ liệu tiếng Việt đúng. ![picParameterGroup](images/picParameterGroup.jpg)
3. Trong **EC2 → Security Groups**, tạo `neonfoodmap-rds-sg`. Inbound chỉ cho phép `MYSQL/Aurora`, port `3306`, với source là **ECS task security group**. Không dùng `0.0.0.0/0` và không dùng IP máy local làm rule production. ![picSecurityGroup](images/picSecurityGroup.jpg)
...
3. Trong **Connectivity**, chọn VPC NeonFoodMap, DB subnet group đã tạo, `Public access = No`, `neonfoodmap-rds-sg` và port 3306. ![picCreateRDS1](images/picCreateRDS1.jpg)
4. Trong **Additional configuration**, đặt initial database `buocchancoi_db`, chọn parameter group; bật automated backup 7 ngày, encryption bằng `aws/rds`, Error log và Slow query log. ![picCreateRDS2](images/picCreateRDS2.jpg)
5. Chọn **Create database**, đợi trạng thái `Available`. Tại **Connectivity & security**, lấy endpoint và lưu vào secret `DB_HOST`.

![picFinalRDS1](images/picFinalRDS1.jpg)

![picFinalRDS2](images/picFinalRDS2.jpg)
