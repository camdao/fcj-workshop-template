...
2. Enable **Use service auto scaling**. Set the minimum task count to `2` and the maximum to `6` so the backend always has two ready tasks while controlling cost. ![picECSScaling2](images/picECSScaling2.jpg)
...
5. Set the scale-out cooldown to `60 seconds` and the scale-in cooldown to `300 seconds`, then save the policy. This lets new tasks register before the next scale-out and prevents rapid scale changes. ![picECSScaling45](images/picECSScaling45.jpg)
