...
| Python                | Python 3.12 +`pip` cho Django backend local development.                |
| Node.js               | Node.js 20+ để build React/Vite frontend (static assets).               |
| Docker                | Build/test backend Django image để push ECR.                            |
...
Template CloudFormation tạo ba nhóm:
- `NeonFoodmap-DevOps-Admins`: Quản lý infrastructure, ECS, RDS, networking
- `NeonFoodmap-Backend-Devs`: Develop và deploy backend Django API
- `NeonFoodmap-Frontend-Devs`: Build và deploy frontend React SPA lên S3/CloudFront

Các policy tự quản lý và MFA được áp dụng. Mọi thành viên phải bật MFA để có thể sử dụng các dịch vụ được cho phép. Việc này được cưỡng chế bởi policy **Force MFA**.

**Lưu ý về frontend:** Frontend developers không cần quyền ECS/ECR vì frontend được build thành static assets và deploy lên S3. Họ cần quyền S3 (upload files) và CloudFront (create invalidation).
